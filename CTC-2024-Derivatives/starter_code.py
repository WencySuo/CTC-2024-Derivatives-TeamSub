...
class Strategy:
  ...
  def black_scholes(S, K, T, r, sigma, option):
    d_1 = Strategy.d1(S, K, T, r, sigma)
    d_2 = Strategy.d2(S, K, T, r, sigma)
  ...
  def getAllGreeks(self, option_price, asset_price, strike_price, time_expire, risk_free_rate, action):
    ...
    #we have all data to find greeks
    delta = self.delta(asset_price, strike_price, time_expire, risk_free_rate, sigma, action)
    vega = self.vega(asset_price, strike_price, time_expire, risk_free_rate, sigma, action)
    theta = self.theta(asset_price, strike_price, time_expire, risk_free_rate, sigma, action)
    rho = self.rho(asset_price, strike_price, time_expire, risk_free_rate, sigma, action)
    gamma = self.gamma(asset_price, strike_price, time_expire, risk_free_rate, sigma, action)
  ...
  #This function takes into account the strike price and the spx price to quote optimal bid/ask sizes
  def order_size_wrt_spx(self, spx_price, strike_price):
    ...
    return np.round(ratio * stock_sz_multiplier, 0)
  ...
  def make_order(self, option_row, spx_price ):
    ...
    if (weighted_ratio < 0.5): #if midprice closer to bid lots of buying pressure, so BUY
      ...
      # order size will be min of g(x) and asking price
      size = min(option_row["ask_sz_00"], self.order_size_wrt_spx(spx_price ,strike_price))

      order = self.new_order(option_row["ts_recv"], option_row['symbol'], 'B', size)

      # slippage is positive so rasises price by .1 percent of order size
      slippage = size * .001
      price = option_row["ask_px_00"] + slippage
      #orders.append(order, ignore_index = True) should be after modifying portfolio and checking
      return [order, price]
    else: #if midprice closer to bid lots of selling pressure, so SELL 

      size = min(option_row["bid_sz_00"], self.order_size_wrt_spx(spx_price ,strike_price))
      order = self.new_order(option_row["ts_recv"], option_row['symbol'], 'S', size)
