...
class Strategy:
  ...
  def implied_vol(self, option_price, asset_price, strike_price, time_expire, risk_free_rate, action):
    ...
    sigma_calc = self.find_sigma_with_scipy(option_price, asset_price, strike_price, time_expire, risk_free_rate, action)
  ...
  def make_order(self, option_row, spx_price ):
    ...
    # Determine the available size (minimum of bid and ask sizes)
    available_size = min(option_row["ask_sz_00"], option_row["bid_sz_00"])
  ...
  # vectorized update_portfolio for optimization 
  def update_portfolio(self, open_orders, spx_price, date, risk_free_rate):
    # Extract necessary data
    symbols = open_orders['option_symbol'].values
    dates = np.array([self.parse_option_symbol(sym)[0] for sym in symbols])
    actions = np.array([self.parse_option_symbol(sym)[1] for sym in symbols])
    strike_prices = np.array([self.parse_option_symbol(sym)[2] for sym in symbols])
    
    # Vectorized calculations
    time_expire = (dates - pd.Timestamp(date)).days.astype(float) / 365
    option_prices = np.array([self.get_last_option_price(sym, date) for sym in symbols])
    
    # Vectorized Greek calculations
    sigmas = np.array([self.find_sigma_with_scipy(op, spx_price, sp, te, risk_free_rate, act) 
                       for op, sp, te, act in zip(option_prices, strike_prices, time_expire, actions)])
    
    deltas = self.delta(spx_price, strike_prices, time_expire, risk_free_rate, sigmas, actions)
    vegas = self.vega(spx_price, strike_prices, time_expire, risk_free_rate, sigmas, actions)
    thetas = self.theta(spx_price, strike_prices, time_expire, risk_free_rate, sigmas, actions)
    rhos = self.rho(spx_price, strike_prices, time_expire, risk_free_rate, sigmas, actions)
    gammas = self.gamma(spx_price, strike_prices, time_expire, risk_free_rate, sigmas, actions)
    
    # Update DataFrame
    open_orders[['sigma', 'delta', 'vega', 'theta', 'rho', 'gamma']] = np.column_stack((sigmas, deltas, vegas, thetas, rhos, gammas))
    
    return open_orders

  # this function updates the portfolio with information from the new spx open price
  def update_portfolio_old(self, open_orders_with_greeks, spx_price, date, risk_free_rate):
